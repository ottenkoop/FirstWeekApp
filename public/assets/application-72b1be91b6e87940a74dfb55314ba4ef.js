...
(function() {
  var SprintToggle, UserstoryPoints;

  SprintToggle = (function() {
    function SprintToggle(el) {
      this.items = $(el);
      this.userstories = this.items.closest('.sprintsoverview').find('.userstories');
      this.bindListeners();
    }

    SprintToggle.prototype.bindListeners = function() {
      return this.items.on('click', (function(_this) {
        return function(e) {
          var target;
          target = $(e.target);
          _this.setCurrent(target);
          return ($(".selectpointsfield")).find("input").trigger("change");
        };
      })(this));
    };

    SprintToggle.prototype.setCurrent = function(index) {
      var userstory;
      userstory = index.closest('.sprintsoverview').find('.userstories');
      ($(this.userstories)).not(userstory).removeClass('current');
      return ($(userstory)).toggleClass('current', ' ');
    };
  ...
  UserstoryPoints = (function() {
    function UserstoryPoints(el) {
      this.userstories = $(el);
      this.userstoryPoints = this.userstories.find('.selectpointsfield').find('input');
      this.copytoclipboard = this.userstories.find('.copytoclipboard');
      this.bindListeners();
    }

    UserstoryPoints.prototype.bindListeners = function() {
      this.userstoryPoints.on('change', function(e) {
        var filled_points, newpointsDiv, pointsLeft, target, totalPoints, totalPointsDiv, userstoryPoints;
        target = $(e.target);
        filled_points = 0;
        if (target.closest('.userstories').hasClass('current')) {
          ...
          userstoryPoints.each(function() {
            return filled_points += Number(($(this)).val());
          });
          pointsLeft = totalPoints - filled_points;
          return newpointsDiv.text(pointsLeft + ' ' + 'punten');
        }
      });
      return this.copytoclipboard.on('click', function(e) {
        var output, target;
        target = $(e.target);
        output = $("<div />");
        this.userstories = target.closest('.userstories').find('.userstoriescontent');
        return target.zclip({
          path: 'http://davidwalsh.name/demo/ZeroClipboard.swf',
          beforeCopy: this.userstories.each(function() {
            return output.append($(this).text()).append(' (' + $(this).find('.userstorypoints').val() + ')' + '\n');
          }),
          copy: function() {
            return output.text();
          }
  ...
  $(function() {
    var sprint_toggle, userstory_points;
    sprint_toggle = new SprintToggle($('#sprints .sprint'));
    return userstory_points = new UserstoryPoints($('#sprints .userstories'));
  });
...
(function() {
  $(function() {
    ...
    return $(".chosen-container").addClass("heightFix");
  });
